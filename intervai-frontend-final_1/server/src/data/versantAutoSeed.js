...
const READING_PASSAGES = [
  {
    title: "Wireless Power Transfer", category: "Technology", difficulty: "medium",
    passage: "Wireless power transfer transmits electricity without physical connections through electromagnetic fields. Applications include smartphone charging pads, electric vehicle charging, and medical implants. Technologies include inductive coupling for short distances and resonant coupling extending range. Future possibilities include room-sized charging zones eliminating cables entirely. However, efficiency decreases with distance, safety concerns about electromagnetic exposure persist, and standardization challenges hinder adoption. Despite limitations, wireless power promises convenience and enables applications impossible with wired connections.",
    questions: [
      ...
      { question: "What is a major limitation of wireless power transfer?", options: [{ id: "A", text: "It only works indoors" }, { id: "B", text: "Efficiency decreases with distance" }, { id: "C", text: "It requires special metals" }, { id: "D", text: "It only charges one device" }], correctOptionId: "B", explanation: "The passage explicitly states efficiency decreases with distance." },
      { question: "What future possibility is mentioned?", options: [{ id: "A", text: "Wireless internet" }, { id: "B", text: "Room-sized charging zones" }, { id: "C", text: "Wireless water supply" }, { id: "D", text: "Satellite charging" }], correctOptionId: "B", explanation: "Room-sized charging zones are mentioned as a future possibility." },
      { question: "What challenge hinders adoption?", options: [{ id: "A", text: "High cost of copper" }, { id: "B", text: "Lack of consumer interest" }, { id: "C", text: "Standardization challenges" }, { id: "D", text: "Government regulations" }], correctOptionId: "C", explanation: "Standardization challenges are explicitly mentioned." },
    ],
  },
  {
    title: "Coral Bleaching Crisis", category: "Environment", difficulty: "hard",
    passage: "Coral bleaching occurs when water temperatures rise, causing corals to expel the symbiotic algae living in their tissues. Without these algae, corals turn white and become vulnerable to disease and death. The Great Barrier Reef has experienced multiple mass bleaching events in recent decades, driven primarily by climate change. Ocean acidification compounds the problem by weakening coral skeletons. While some corals can recover if temperatures normalize quickly, repeated bleaching events prevent full recovery. Scientists are exploring heat-resistant coral strains and assisted evolution techniques.",
    questions: [
      ...
      { question: "What compounds the bleaching problem?", options: [{ id: "A", text: "Ocean acidification" }, { id: "B", text: "Plastic pollution" }, { id: "C", text: "Noise pollution" }, { id: "D", text: "Oil spills" }], correctOptionId: "A", explanation: "Ocean acidification weakens coral skeletons." },
      { question: "What are scientists exploring to help corals?", options: [{ id: "A", text: "Artificial coral reefs" }, { id: "B", text: "Heat-resistant coral strains" }, { id: "C", text: "Underwater cooling systems" }, { id: "D", text: "Chemical treatments" }], correctOptionId: "B", explanation: "Scientists are exploring heat-resistant coral strains." },
  ...
  {
    title: "Brain-Computer Interfaces", category: "Technology", difficulty: "hard",
    passage: "Brain-computer interfaces (BCIs) create direct communication pathways between the brain and external devices. Invasive BCIs, like Neuralink's implants, offer high signal resolution but require surgery. Non-invasive options like EEG headsets are safer but less precise. Medical applications include restoring movement to paralyzed patients and treating neurological disorders. BCIs have enabled paralyzed individuals to control robotic arms and communicate through thought alone. Ethical concerns include privacy of thoughts, potential for cognitive enhancement inequality, and the blurring of human-machine boundaries.",
    questions: [
      ...
      { question: "What is a disadvantage of invasive BCIs?", options: [{ id: "A", text: "Low signal resolution" }, { id: "B", text: "High cost" }, { id: "C", text: "Require surgery" }, { id: "D", text: "Limited range" }], correctOptionId: "C", explanation: "Invasive BCIs require surgery." },
...
const LISTENING_PASSAGES = [
  {
    ...
    questions: [
      ...
      { question: "Where does confirmation bias affect decision-making?", options: [{ id: "A", text: "Only in politics" }, { id: "B", text: "Only in medicine" }, { id: "C", text: "In everyday life including politics and medicine" }, { id: "D", text: "Only in academic settings" }], correctOptionId: "C", explanation: "It affects decision-making in everyday life." },
  ...
  },
  {
    ...
    questions: [
      ...
      { question: "What do growth mindset individuals do with challenges?", options: [{ id: "A", text: "Avoid them" }, { id: "B", text: "Embrace them as learning opportunities" }, { id: "C", text: "Ignore them" }, { id: "D", text: "Delegate them" }], correctOptionId: "B", explanation: "Growth mindset individuals embrace challenges as learning opportunities." },
      { question: "What do schools with growth mindset programs see?", options: [{ id: "A", text: "Higher dropout rates" }, { id: "B", text: "Improved student resilience" }, { id: "C", text: "Lower test scores" }, { id: "D", text: "More absenteeism" }], correctOptionId: "B", explanation: "Schools see improved student resilience and academic performance." },
    ],
  },
  {
    title: "Financial Literacy Importance", category: "Economics", difficulty: "easy",
    passage: "Financial literacy — the ability to understand and effectively use financial skills — is increasingly recognized as essential for personal and societal wellbeing. Studies show that financially literate individuals make better investment decisions, accumulate more wealth, and are better prepared for retirement. Despite its importance, financial literacy rates remain low globally. Schools are beginning to incorporate personal finance education, but coverage remains inconsistent. Digital tools and apps are democratizing access to financial education.",
    questions: [
      ...
      { question: "What challenge exists with school financial education?", options: [{ id: "A", text: "Too expensive" }, { id: "B", text: "Coverage remains inconsistent" }, { id: "C", text: "Students refuse to learn" }, { id: "D", text: "No qualified teachers" }], correctOptionId: "B", explanation: "Coverage of personal finance education in schools remains inconsistent." },
    ],
  },
];
...
async function autoSeedVersant() {
  try {
    const count = await VersantQuestion.countDocuments();
    if (count > 0) {
      console.log(`Versant: ${count} questions already in DB, skipping seed.`);
      return;
    }

    console.log("Versant: No questions found — auto-seeding...");

    const grammarDocs = GRAMMAR.map((g) => ({ type: "grammar", difficulty: g.difficulty, topic: "Grammar", category: "English", sentence: g.sentence, options: g.options, correctOptionId: g.correctOptionId, explanation: g.explanation }));
    const readingDocs = READING_PASSAGES.map((r) => ({ type: "reading", difficulty: r.difficulty, topic: r.title, category: r.category, title: r.title, passage: r.passage, questions: r.questions }));
    const listeningDocs = LISTENING_PASSAGES.map((l) => ({ type: "listening", difficulty: l.difficulty, topic: l.title, category: l.category, title: l.title, passage: l.passage, questions: l.questions }));
    const speakingDocs = SPEAKING_PROMPTS.map((s) => ({ type: "speaking", difficulty: s.difficulty, topic: s.topic, category: "Speaking", prompt: s.prompt, sampleAnswer: s.sampleAnswer }));

    const all = [...grammarDocs, ...readingDocs, ...listeningDocs, ...speakingDocs];
    await VersantQuestion.insertMany(all);
    console.log(`Versant: Seeded ${all.length} questions successfully.`);
  } catch (err) {
    console.error("Versant auto-seed error:", err.message);
  }
}

module.exports = autoSeedVersant;
